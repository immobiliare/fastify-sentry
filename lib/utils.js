...
const extractPathForTransaction = (request, getName = getTransactionName) => {
  ...
  let source = 'url';
  // TODO: see if there's a way in fastify to attach a boolean to the request
  // to pass this information (that te request has params).
  if (Object.keys(request.params) > 0) {
    source = 'route';
  }
  return [name, source];
};
